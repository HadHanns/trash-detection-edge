"""
train_yolov8n.py  —  Training YOLOv8n (100 epoch) dengan resume support
...
MODEL      = "yolov8n.pt"                              # ← YOLOv8n
EPOCHS     = 100
BATCH      = 4       # GTX 970 4GB; turunkan ke 2 jika OOM
IMGSZ      = 640
DEVICE     = 0
SAVE_DIR   = Path("C:/yolo_out/rft_yolov8n")          # ← folder output terpisah
...
best_fitness = 0.0
best_pt_path = SAVE_DIR / "weights" / "best_yolov8n.pt"
last_pt_path = SAVE_DIR / "weights" / "last_yolov8n.pt"
...
def save_on_epoch_end(trainer):
    ...
    try:
        ...
        print(f"\n[SAVE ✅] last_yolov8n.pt tersimpan (epoch {epoch}/{EPOCHS})")
        ...
        if fitness is not None and fitness >= best_fitness:
            ...
            print(f"[SAVE ✅] best_yolov8n.pt diperbarui (fitness={fitness:.4f})")
...
def main():
    ...
    if can_resume:
        try:
            ...
            print(f"[RESUME] ✅ Checkpoint ditemukan: last_yolov8n.pt (epoch {last_ep})")
    ...
    if not can_resume:
        print(f"[INFO] Mulai training YOLOv8n dari awal (epoch 1/{EPOCHS})")
    ...
    if src.exists():
        dest = OUTPUT_DIR / "best_yolov8n.pt"
