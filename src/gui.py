...
class TrashDetectionApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(
            top_bar, text="🗑️  Kaggle RFT Trash Detection (8 Classes) — YOLO11n",
            bg=BG_CARD, fg=TEXT_MAIN, font=("Segoe UI", 12, "bold")
        ...
        self._status_bar = tk.Label(
            ...
            bg=BG_CARD, fg=TEXT_SUB, font=("Segoe UI", 9), anchor="w",
        )
    ...
    def _build_left_panel(self, parent):
        ...
        canvas_scroll = tk.Canvas(wrapper, bg=BG_MID, highlightthickness=0, yscrollcommand=sb.set)
        ...
        img_frame = tk.LabelFrame(p, text=" 📁 GAMBAR ", bg=BG_MID, fg=TEXT_MAIN,
                                  font=("Segoe UI", 9, "bold"), bd=1, padx=8, pady=8)
        ...
        tk.Label(img_frame, textvariable=self._filename_var, bg=BG_MID, fg=TEXT_SUB,
                 font=("Segoe UI", 8), wraplength=210).pack(anchor="w", padx=4)
        ...
        tk.Label(vid_frame, textvariable=self._video_progress_var,
                 bg=BG_MID, fg=TEXT_SUB, font=("Consolas", 8)).pack(anchor="w")


        # ══════════════════════════════════════════════════════════════════════
        # SEKSI ROI (Region of Interest)
        # ══════════════════════════════════════════════════════════════════════
        roi_frame = tk.LabelFrame(p, text=" 📐 AREA DETEKSI (ROI) ", bg=BG_MID, fg=TEXT_MAIN,
                                  font=("Segoe UI", 9, "bold"), bd=1, padx=8, pady=8)
        roi_frame.pack(fill="x", padx=12, pady=6)
        
        self._roi_btn = ttk.Button(roi_frame, text="Tentukan ROI", style="Accent.TButton", command=self._toggle_roi_draw)
        ...
        tk.Label(roi_frame, textvariable=self._roi_status_var, bg=BG_MID, fg=TEXT_SUB, font=("Segoe UI", 8)).pack(anchor="w", pady=(4, 0))
        ...
        ttk.Separator(set_frame, orient="horizontal").grid(row=2, column=0, columnspan=2, sticky="ew", pady=6)
    ...
    # (Removed unused _section_header, _section_subheader, _divider since we use LabelFrame now)
    ...
    def _toggle_roi_draw(self):
        ...
        if self._is_drawing_roi:
            ...
            self._roi_status_var.set(f"Status: Aktif ({len(self.roi_points)} titik)" if len(self.roi_points) >= 3 else "Status: Tidak aktif")
            self._canvas_orig.unbind("<Button-1>")
            self._canvas_orig.unbind("<Button-3>")
            self._canvas_video.unbind("<Button-1>")
            self._canvas_video.unbind("<Button-3>")
    ...
    def _redraw_roi_on_orig(self):
        ...
        if tab_idx == 0 and self.img_bgr is not None:
            img = self.img_bgr
            canvas = self._canvas_orig
            tag = "orig"
        elif tab_idx == 3 and getattr(self, '_video_last_frame', None) is not None:
            img = self._video_last_frame
            canvas = self._canvas_video
            tag = "video"
        else:
            return
        ...
        if len(self.roi_points) > 0:
            draw = ImageDraw.Draw(pil_img, "RGBA")
            if len(self.roi_points) >= 3:
                draw.polygon(self.roi_points, outline=(0, 255, 255, 255), width=3, fill=(0, 255, 255, 40))
            else:
                for pt in self.roi_points:
                    r = 4
                    draw.ellipse([pt[0]-r, pt[1]-r, pt[0]+r, pt[1]+r], fill=(0, 255, 255, 255))
    ...
    def _upload_video(self):
        ...
        ret, frame = cap.read()
        if ret: self._video_last_frame = frame.copy()
        if ret:
    ...
    def _video_loop(self):
        ...
        while self._video_running:
            ...
            pil_out = draw_boxes_pil(Image.fromarray(img_rgb), boxes, scores, cls_ids, roi_points=getattr(self, "roi_points", []))
    ...
    def _finish_yolo(self, img_bgr, boxes, scores, cls_ids, ms):
        ...
        img_rgb  = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
        pil_img  = Image.fromarray(img_rgb)
        pil_res  = draw_boxes_pil(pil_img.copy(), boxes, scores, cls_ids, roi_points=getattr(self, 'roi_points', []))
