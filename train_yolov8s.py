"""
train_yolov8s.py  —  Training YOLOv8s (100 epoch) dengan resume support
...
- Internal Ultralytics save=False (hindari crash), save manual via callback
"""
...
MODEL      = "yolov8s.pt"                              # ← YOLOv8s
EPOCHS     = 100
BATCH      = 4       # GTX 970 4GB; turunkan ke 2 jika OOM
IMGSZ      = 640
DEVICE     = 0
SAVE_DIR   = Path("C:/yolo_out/rft_yolov8s_9class")   # ← folder output terpisah untuk 9 kelas
...
best_fitness = 0.0
best_pt_path = SAVE_DIR / "weights" / "best_yolov8s.pt"
last_pt_path = SAVE_DIR / "weights" / "last_yolov8s.pt"

# ── Info ─────────────────────────────────────────────────────────────────────
print(f"[INFO] Model   : {MODEL}")
print(f"[INFO] Dataset : Kaggle RFT (9 kelas)")
print(f"[INFO] Kelas   : bottle, grass, branch, milk-box, plastic-bag, plastic-garbage, ball, leaf, pile")
...
def save_on_epoch_end(trainer):
    ...
    try:
        ...
        ckpt = {
            "epoch"      : epoch,
            "model"      : deepcopy(trainer.model).cpu(),
        ...
        print(f"\n[SAVE ✅] last_yolov8s.pt tersimpan (epoch {epoch}/{EPOCHS})")
        ...
        if fitness is not None and fitness >= best_fitness:
            ...
            print(f"[SAVE ✅] best_yolov8s.pt diperbarui (fitness={fitness:.4f})")
...
def main():
    ...
    if can_resume:
        try:
            ...
            print(f"[RESUME] ✅ Checkpoint ditemukan: last_yolov8s.pt (epoch {last_ep})")
            print(f"[RESUME] Training akan lanjut dari epoch {last_ep} → {EPOCHS}")
        except Exception:
            print("[RESUME] ⚠️  Gagal baca last_yolov8n.pt, mulai dari awal.")
            can_resume = False

    if not can_resume:
        print(f"[INFO] Mulai training YOLOv8s dari awal (epoch 1/{EPOCHS})")
    ...
    if can_resume:
        ...
        print("[RESUME] Model dimuat dari last_yolov8n.pt")
    else:
        model = YOLO(MODEL)
    ...
    print("[INFO] Mulai training (save=False → checkpoint manual via callback)...")
    try:
        results = model.train(
            data       = str(DATA_YAML),
            epochs     = EPOCHS,
    ...
    if src.exists():
        dest = OUTPUT_DIR / "best_yolov8s.pt"
